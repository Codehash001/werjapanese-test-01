...
contract WeRjapanese01 is ERC721A, Ownable, ReentrancyGuard {
  using Strings for uint256;

  string public baseURI;
  string public baseExtension = ".json";
  string public notRevealedUri = "ipfs://QmNrpvQmPkKTjPneKwhcuQwDpcXs6Pcce4sUsikjcvEeUT/weRjapanese_nonRevealed_metadata.json"; 

  uint256 public cost = 0.01 ether;
  uint256 public wlcost = 0.01 ether;
  uint256 public maxSupply = 20;
  uint256 public WlSupply = 10;
  ...
  function publicSaleMint(uint256 tokens) public payable nonReentrant {
    ...
    require(tokens <= MaxperWallet, "WRJ: max mint amount per tx exceeded");
    require(supply + tokens <= maxSupply, "WRJ: We Soldout");
    require(_numberMinted(_msgSender()) + tokens <= MaxperWallet, "WRJ: Max NFT Per Wallet exceeded");
